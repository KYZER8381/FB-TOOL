...
def AutoReact():
    def Reaction(actor_id: str, post_id: str, react: str, token: str) -> bool:
        ...
        try:
            if react == '0':  # Removing reaction
                print(f"{g}「Success」» Removed reaction from {actor_id} on {post_id}")
                return True
            elif react in str(pos):  # Reaction successful
    ...
    if reaction_ids:
        ...
        # Create an accurate reaction queue based on the desired count
        total_reactions = min(react_count, len(actor_ids) * len(reaction_ids))  # Ensure you don't exceed available reactions
        reactions_per_type = total_reactions // len(reaction_ids)
        reaction_queue = [react for react in reaction_ids for _ in range(reactions_per_type)]
        remaining_reactions = total_reactions - len(reaction_queue)  # Add remaining reactions if any
        reaction_queue.extend([reaction_ids[-1]] * remaining_reactions)  # Handle remainders by using the last reaction
